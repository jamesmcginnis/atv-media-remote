...
if (this._remoteMode) {
  ...
  cardOuter.classList.add('r-remote-active');
} else {
  overlay.classList.add('hidden');
  remoteBtn.classList.remove('remote-btn-active');
  cardOuter.classList.remove('r-remote-active');
...
.size-toggle svg { width: 18px; height: 18px; fill: rgba(255,255,255,0.78); }
.size-toggle:active, .size-toggle.pressed { transform: scale(0.9); box-shadow: 0 0 16px rgba(255,255,255,0.2); }
/* hide floating toggle when remote overlay is open — inline one takes over */
.r-remote-active .size-toggle { display: none; }
...
```
    ...
    .mode-compact .size-toggle { top: 8px; right: 8px; width: 28px; height: 28px; background: rgba(0,0,0,0.45); }
    ...
    /* Inline size-toggle inside r-top-row — only shown in remote mode */
    .r-mode-btn {
      margin-left: auto;
      ...
      backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
      border: 1px solid rgba(255,255,255,0.22) !important;
      cursor: pointer; flex-shrink: 0; transition: all 0.2s ease; padding: 0;
    }
    .r-mode-btn svg { width: 18px; height: 18px; fill: rgba(255,255,255,0.78); }
    .r-mode-btn:active, .r-mode-btn.pressed { transform: scale(0.9); box-shadow: 0 0 16px rgba(255,255,255,0.2); }
      ...
      <div class="remote-overlay hidden" id="remoteOverlay">
        <div class="remote-panel">
          ...
          <div class="r-top-row">
            ...
            <button class="r-mode-btn" id="rModeBtn">
              <svg viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.78)" stroke-width="3">
...
const toggleCompact = () => r.getElementById('cardOuter').classList.toggle('mode-compact');
r.getElementById('modeBtn').onclick = toggleCompact;
r.getElementById('rModeBtn').onclick = (e) => { e.stopPropagation(); toggleCompact(); };
addPressEffect(r.getElementById('rModeBtn'));
