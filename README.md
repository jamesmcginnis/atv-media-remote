...
- **Apple TV remote control** — rounded rectangle touchpad with Up / Down / Left / Right / Select navigation, Back, TV home, and Power on/off with live state indicator
...
4. Add `https://github.com/jamesmcginnis/atv-media-remote` and select **Dashboard** as the category
5. Click **Install**
...
2. Click **+ Add Card**
3. Scroll to the bottom and select **ATV Media Remote**
4. Configure your entities and preferences using the visual editor
5. Click **Save**
...
The card sits as a slim strip showing the album art thumbnail, track title, artist, a progress bar, playback controls, and volume. This is the recommended mode for dashboards where space is limited.

Every time you navigate back to the page or reload the app, the card returns to whichever view you have set as the **Startup View** — so it always opens consistently.
...
| **Power** | `turn_on` / `turn_off` | Toggle Apple TV power. Glows red when the device is on |
...
When a `remote.*` entity is found matching your `media_player.*` entity name, the volume control sends `volume_up` and `volume_down` remote commands. This works reliably in every app including YouTube, Infuse, Plex, Netflix, and Disney+ — apps that manage their own audio pipeline and ignore the standard `volume_set` service.

The slider uses a throttled approach — one command fires immediately on first movement, then one per ~380ms while dragging continues, with a trailing command to ensure the final position always registers. This gives a responsive feel while preventing commands from being dropped by the integration.

**Standard media players**

For any player without a matched remote entity, the volume control uses `media_player.volume_set` for precise absolute control, exactly as normal.

**Slider vs Buttons**

In the visual editor, toggle **Use Volume Buttons** to switch between:

- **Slider** — a full-width drag bar that sits on its own row below the playback controls, aligned with the progress bar
...
The card snaps back to this view every time you navigate to the page or reload the app. Changing the setting in the editor takes effect immediately — the card transitions to the new view without a page reload.
...
- Preserve manual selections — once you choose a device manually, auto-switch will not override it until you switch away or that device becomes inactive

To reorder entities in the selector, open the visual editor and drag the handles next to each device name.

---

## Visual Editor

Open the card editor by clicking the pencil icon while in dashboard edit mode.
